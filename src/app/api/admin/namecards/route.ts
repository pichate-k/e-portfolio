...
async function ensureDefaultCardExists() {
  ...
  await prisma.namecard.create({
    data: {
      ...
      organization: profile?.workplace || "Faculty of Engineering, Rajamangala University of Technology Thanyaburi",
      organizationTh: profile?.workplaceTh || "คณะวิศวกรรมศาสตร์ มหาวิทยาลัยเทคโนโลยีราชมงคลธัญบุรี",
  ...
  try {
    ...
    const newCard = await prisma.namecard.create({
      data: {
        ...
        fullName: data.fullName?.trim() || "Dr. Pichate K.",
